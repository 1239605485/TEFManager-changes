...
class ModOverlaySettingsService : Service() {
    ...
    private fun leftGame(): Boolean {
        ...
        return when (last) {
            gamePackage -> { seenGame = true; lastGameForegroundAt = now; false }
            null -> false
            else -> seenGame && lastGameForegroundAt > 0L && now - lastGameForegroundAt > 8_000L
        }
    }
    ...
    private fun hideBubble() { remove(bubble); bubble = null; if (hotspot != null) return
        val v = View(this).apply { alpha = .01f; setOnClickListener { remove(hotspot); hotspot = null; showPanel() } }; hotspot = v
    ...
    private fun settings(shell: LinearLayout, entry: OverlayMod, restoreScroll: Int = 0) { shell.removeAllViews(); shell.addView(header(entry.mod.name) { list(shell) }); shell.addView(label("修改后立即保存并应用",10,0xFF6B6B83.toInt()).apply { setPadding(dp(4),0,0,dp(5)) })
        ...
        shell.addView(scroll,LinearLayout.LayoutParams(-1,dp(238)))
    ...
    private fun row(e:OverlayMod,s:ModItem.ModSetting,current:JsonElement):View { val r=LinearLayout(this).apply { orientation=LinearLayout.HORIZONTAL; gravity=Gravity.CENTER_VERTICAL; setPadding(dp(7),dp(5),dp(5),dp(5)); background=shape(LAVENDER,9) }
        ...
        when(s.type) { ModItem.SettingType.SWITCH -> r.addView(Switch(this).apply { scaleX=.72f;scaleY=.72f;isChecked=current.jsonPrimitive.booleanOrNull?:false;setOnCheckedChangeListener{_,v->save(e,s.key,JsonPrimitive(v))} },LinearLayout.LayoutParams(dp(46),dp(30)))
            ...
            ModItem.SettingType.CHOICE -> r.addView(
                button(s.options.firstOrNull { it.value==current.jsonPrimitive.content }?.label?:current.jsonPrimitive.content) {
